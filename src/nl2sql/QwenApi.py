...
class QwenApi:
    ...
    def answer(self, query, md):
        ...
        2. 推理过程要列出和答案相关的招生信息，如专业，分数，最低位次号等，不要将无关的信息放到推理过程中，但要尽量简洁
        3. 最终结论要简洁明了，尽可能给出结论性的建议, 不做过多解释
        4. 如果最低分和最低位次号有冲突，以最低分为依据

        【示例】
        用户问题：浙江530分可以选择什么专业？
        回答： <REASONING>根据历年信息，浙江2023年最低分小于530分的专业有：专业1(最低分xxx分)，专业2（最低分xxx分）; 浙江2024年最低分小于530分的专业有：专业1(最低分xxx分); </REASONING>\n<SOLUTION>可以选择的专业有：专业A(最低分525分)，专业B(最低分510分)</SOLUTION>
        """
